...
vector<vector<int>> strassens_multiply(vector<vector<int>> a, vector<vector<int>> b, int n)
{
    ...
    vector<int> f(si, 0);
    vector<vector<int>>
        c11(si, f), c12(si, f), c21(si, f), c22(si, f), a11(si, f), a12(si, f), a21(si, f), a22(si, f), b11(si, f), b12(si, f), b21(si, f), b22(si, f), p1(si, f), p2(si, f), p3(si, f), p4(si, f), p5(si, f), p6(si, f), p7(si, f);
    for (int i = 0; i < si; i++)
    {
        for (int j = 0; j < si; j++)
        {
...
{
    int r1, c1, r2, c2;
    cout << "Enter the No of Rows in m1 : ";
    cin >> r1;
    cout << "Enter the No of Columns in m1 : ";
    cin >> c1;
    cout << "Enter the No of Rows in m2 : ";
    cin >> r2;
    cout << "Enter the No of Columns in m2 : ";
    cin >> c2;
    ...
    double runtime = duration_cast<nanoseconds>(end - start).count();
    runtime *= 1e-9;
    cout << "Time Taken : " << fixed << setprecision(8) << runtime << "seconds";
    return 0;
}